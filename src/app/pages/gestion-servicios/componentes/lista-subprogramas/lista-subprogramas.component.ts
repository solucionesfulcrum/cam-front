...
@Component({
  ...
  templateUrl: './lista-subprogramas.component.html',
  styleUrls: ['./lista-subprogramas.component.scss']
})
export class ListaSubprogramasComponent {
  ...
  faSpinner = faSpinner;
  dataSource: SubprogramaListadoItem[] = [];
  pageIndex = 0;
  pageNum = 1;
  pageSize = 10;
  ...
  afectarTodo(evento: Event): void {
    ...
    this.seleccionados = element.checked
      ? this.dataSource.map((data) => data.idSubPrograma)
      : [];
  }
